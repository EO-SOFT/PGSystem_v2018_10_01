...
public class ConfigProject extends DAO implements Serializable{
    ...
    public List<String[]> selectHarnessTypeByProject(String customer) {
        Helper.startSession();
        SQLQuery query = Helper.sess.createSQLQuery("SELECT DISTINCT harness_type FROM config_project WHERE customer = '"+customer+"'");
     ...
     * @return 
     */
    public ConfigProject getProjectByHarnessType(String harnessType) {
        Helper.startSession();
        Query query = Helper.sess.createQuery(HQLHelper.GET_PROJECT_BY_HARNESSTYPE);
        query.setParameter("harnessType", harnessType);
        Helper.sess.getTransaction().commit();
        return (ConfigProject)query.list().get(0);
    }
