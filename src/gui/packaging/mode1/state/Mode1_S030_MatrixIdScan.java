...
public class Mode1_S030_MatrixIdScan implements Mode1_State {
    ...
    public void doAction(Mode1_Context context) {
        ...
        if (!bh.checkDataMatrixFormat(counter)) {//Problème de format
            ...
            UILog.info("Valid Harness Counter scanned [" + counter + "] OK.");
...
//                UILog.info("Hostname check OK");
//                UILog.info("Pallet values ");
//                UILog.info(String.format("State           :   [%s]", bc.getContainerState().equals(GlobalVars.PALLET_OPEN)));
//                UILog.info(String.format("Qty Expected    :   [%s]", bc.getQtyExpected()));
//                UILog.info(String.format("Qty Read        :   [%s]", bc.getQtyRead()));
//                UILog.info(String.format("Std Time        :   [%s]", bc.getStdTime()));
//                UILog.info(String.format("Harness Part [%s] = Context Harness Part [%s]",
//                        bc.getHarnessPart(),
//                        PackagingVars.mode1_context.getTempBC().getHarnessPart()));
//                UILog.info(String.format("Harness Type [%s] = Context Harness Type [%s]",
...
//                query.executeUpdate();
    ...
    private void setToWaiting(BaseContainer bc, int newQty) {
        UILog.info(String.format("Quantité terminée %s", bc.toString()));
        try {
            PrinterHelper.saveAndPrintClosingSheetMode1(PackagingVars.mode1_context, bc, false);
